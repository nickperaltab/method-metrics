...
1. **Untagged repeat visits are discarded.** `CookieTracker.cs:165`, `if (TrackingOn || isNew)`. A click without a `trc` code is recorded only on a browser's first ever visit, for the 365-day cookie life. Untagged cookies average 1.00 clicks, tagged 1.25. Estimated ~442k clicks never written over 20 months.
   **This is bigger than click volume: it makes multi-touch attribution impossible.** Position in journey, 2025+, measured on RAW `marketing.CampaignCookieClicks` (so this is not an artifact of `int_cookie_clicks` filtering): Facebook Ads 24.3% later-touch, AdWords 22.2%, Bing 14.6% — against SEO 0.7%, Direct 0.2%, AIO 0.2%. Direct at 99.8% first-touch is the proof it is mechanical, since typing a URL from memory is definitionally a return visit. Raw is 1.02 clicks per cookie and 98.8% single-click; 79% of cookies that became customers have exactly one click.
   Consequence: Alocet's multi-touch engine, its U/J/L curves and its Direct-suppression rule all operate on data that has had every touch but the first stripped out. 99.7% of cookies show one channel. And AIO can only ever be credited when an assistant was a browser's first ever contact with Method — a mid-funnel AI consultation, which is the position assistants actually occupy, is discarded. Conversion RATES stay valid because the cohort is coherent; cross-channel VOLUME comparison is biased against every untagged channel.
