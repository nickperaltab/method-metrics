...
export function registerTools(server: McpServer): void {
  server.registerTool(
    "list_metrics",
    {
      title: "List metrics",
      description:
        "List every verified dbt metric model (v_metric__*) with its metric_id, status label, and a one-line description. Optionally filter by status (e.g. 'live'). Read-only; served from the dbt manifest.",
      inputSchema: {
        status: z
          .string()
          .optional()
  ...
  server.registerTool(
    ...
    async () => {
      try {
        ...
        return ok(
          JSON.stringify({ count: intermediates.length, intermediates, missing }, null, 2),
        );
      } catch (err) {
        return fail(err);
      }
  ...
  server.registerTool(
    "get_lineage",
    {
      ...
      description:
        "Walk a metric's upstream dependency chain (via the dbt manifest parent_map) from the metric model through intermediates down to raw sources. Returns an indented text tree. Accepts model name, short name, or metric_id; also resolves allowlisted intermediates. Read-only.",
      inputSchema: {
  ...
  server.registerTool(
    "get_sql",
    {
      title: "Get model SQL",
      description:
        "Get a dbt model's SQL, read from the .sql file on disk (manifest raw_code is not trusted — it can be a placeholder). Works for any model, including intermediates (int_*), not just v_metric__* views. Includes the compiled SQL from target/compiled/ when present, labeled separately. Read-only.",
      inputSchema: {
        model: z
          .string()
