...
4. **`revenue.int_conversions.TotalSaaSAmount` is entity-grain data sitting on account-grain rows — `SUM()` over it overstates by ~2.5x.** The view computes `SUM(SaaSAmount)` per `EntityRecordID` (a customer's lifetime SaaS total from `TransLineFlattened`) and attaches that same total to every `CompanyAccount` row that entity owns — but the view does not project `EntityRecordID`, so there is no key to dedup with. Measured 2026-08-17: naive `SUM(TotalSaaSAmount)` reads **$109.5M**; the correct customer-level total is **$43.7M**. Method Monday only ever `COUNT(*)`s this view, so nothing currently published is wrong — but the next person who writes `SUM(TotalSaaSAmount)` against `revenue.int_conversions` will be. If you need this total, join back to `Account.EntityRecordID` (or wait for the dbt-managed `int_conversions` model, drafted but not yet deployed — see `.superpowers/sdd/2026-08-10-method-monday/audit-dbt-infrastructure.md` §1.4-1.5) and dedup to one row per `EntityRecordID` before summing.
