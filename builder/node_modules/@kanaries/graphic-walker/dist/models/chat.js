...
import { Methods, reducer } from './visSpecHistory';
...
export function toVegaSimplified(chart) {
    ...
    const viewEncodingFields = viewEncodingKeys(geoms[0] ?? 'auto').flatMap((k) => chart.encodings?.[k] ?? []);
    ...
    const specs = toVegaSpec({
        ...
        if (newSpec['data'])
            delete newSpec['data'];
        if (newSpec['resolve'])
            delete newSpec['resolve'];
        if (newSpec['params'])
            delete newSpec['params'];
        if (newSpec['encoding']?.['tooltip'])
            delete newSpec['encoding']['tooltip'];
        if (newSpec['encoding']) {
            newSpec['encoding'] = Object.fromEntries(Object.entries(newSpec.encoding).map(([k, field]) => {
                return [
                    k,
                    {
                        field: field.title,
...
export function toVegaSimplifiedWithAggergation(chart) {
    ...
    const registerField = (field) => {
        ...
        const aggName = shouldAnnotateAggregate ? field.aggName : undefined;
    ...
    const enhanceFieldDef = (fieldDef) => {
        ...
        const next = { ...fieldDef, field: meta.field, title: meta.title };
        if (meta.aggregate) {
    ...
    const visitSpec = (spec) => {
        ...
        if (spec.encoding) {
            spec.encoding = Object.fromEntries(Object.entries(spec.encoding).map(([channel, value]) => {
...
function toMessage(data, action) {
    const [type, ...props] = action;
    return actionMessageMapper[type](data, ...props);
}
export function toChatMessage(history) {
    const result = [];
    ...
    const createGeneratedMessages = () => {
        if (message) {
            ...
            const viewEncodingFields = viewEncodingKeys(now.config.geoms[0] ?? 'auto').flatMap((k) => now.encodings?.[k] ?? []);
