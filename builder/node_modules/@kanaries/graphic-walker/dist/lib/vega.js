...
function buildWindowTransforms({ fields, orderField, groupByFields, defaultAggregated }) {
    if (!defaultAggregated)
        return [];
    const windowFields = fields.filter((field) => field.analyticType === 'measure' &&
    ...
    const addWindow = (field, windowAgg) => {
        ...
        if (windowAgg === 'difference' || windowAgg === 'growth_rate') {
            ...
            if (windowAgg === 'difference') {
                transforms.push({
                    calculate: `datum[${JSON.stringify(baseField)}] === null || datum[${JSON.stringify(lagField)}] === null ? null : datum[${JSON.stringify(baseField)}] - datum[${JSON.stringify(lagField)}]`,
                    as: windowField,
                });
            }
            else {
                transforms.push({
                    calculate: `datum[${JSON.stringify(lagField)}] === null || datum[${JSON.stringify(lagField)}] === 0 ? null : (datum[${JSON.stringify(baseField)}] / datum[${JSON.stringify(lagField)}]) - 1`,
                    as: windowField,
                });
            }
        }
    };
    windowFields.forEach((field) => addWindow(field, field.windowAgg));
    return transforms;
}
export function toVegaSpec({ rows: rowsRaw, columns: columnsRaw, color, opacity, size, details = [], radius, shape, text, theta, interactiveScale, dataSource, layoutMode, width, height, defaultAggregated, geomType, stack, scales, mediaTheme, vegaConfig, displayOffset, }) {
    const guard = defaultAggregated ? (x) => x ?? NULL_FIELD : (x) => (x ? (x.aggName === 'expr' ? NULL_FIELD : x) : NULL_FIELD);
    ...
    if (rowRepeatFields.length <= 1 && colRepeatFields.length <= 1) {
        ...
        ].filter((field) => field !== NULL_FIELD);
        const windowTransforms = buildWindowTransforms({
            fields: [xField, yField, guard(color), guard(opacity), guard(size), guard(shape), guard(theta), guard(radius), guard(text), ...details.map(guard)].filter((field) => field !== NULL_FIELD),
            orderField,
            groupByFields,
            defaultAggregated,
        });
        ...
        if ('layer' in singleView && singleView.layer) {
            ...
            if (spec.params) {
                spec.layer = singleView.layer.map((l, i) => (i === 0 ? { ...l, params: spec.params } : l));
                delete spec.params;
            }
            else {
        ...
        for (let v in resolve) {
            ...
            if (GLOBAL_CONFIG.POSITION_CHANNEL_CONFIG_LIST.includes(v)) {
                spec.resolve.axis = { ...spec.resolve.axis, [v]: value };
            }
            else if (GLOBAL_CONFIG.NON_POSITION_CHANNEL_CONFIG_LIST.includes(v)) {
        ...
        for (let i = 0; i < leastOne(rowRepeatFields.length); i++) {
            for (let j = 0; j < leastOne(colRepeatFields.length); j++, index++) {
                ...
                const xFieldLocal = (colRepeatFields[j] || NULL_FIELD);
                const yFieldLocal = (rowRepeatFields[i] || NULL_FIELD);
                const orderField = xFieldLocal !== NULL_FIELD && xFieldLocal.analyticType === 'dimension'
                    ? xFieldLocal
                ...
                const windowTransforms = buildWindowTransforms({
                    fields: [xFieldLocal, yFieldLocal, guard(color), guard(opacity), guard(size), guard(shape), guard(theta), guard(radius), guard(text), ...details.map(guard)].filter((field) => field !== NULL_FIELD),
                    orderField,
                    groupByFields,
                    defaultAggregated,
                });
