...
function dateTimeDrill(resKey, params, data) {
    const fieldKey = params.find((p) => p.type === 'field')?.value;
    const drillLevel = params.find((p) => p.type === 'value')?.value;
    ...
    const newDate = ((...x) => toOffsetDate(prepareDate(...x)));
    function getISOYear(v) {
    ...
    switch (drillLevel) {
        ...
        case 'week': {
            const newValues = fieldValues.map((v) => {
                ...
                const FirstSundayOfYear = SundayOfFirstWeek.getFullYear() === Y ? SundayOfFirstWeek : newDate(SundayOfFirstWeek.setDate(SundayOfFirstWeek.getDate() + 7));
...
//# sourceMappingURL=dateTimeFeature.js.map