...
function vegaLiteToPlot(spec) {
    ...
    const getMarkDirection = (markType, enc) => {
        ...
        if (directionalMarks[markType]) {
            ...
            if (xIsQuantitative && !yIsQuantitative) {
                ...
                return { markFunction: directionalMarks[markType].X, stackAxis: 'X' };
            }
            ...
            if (yIsQuantitative && !xIsQuantitative) {
                ...
                return { markFunction: directionalMarks[markType].Y, stackAxis: 'Y' };
            }
    ...
    const createBaseConfig = (markType, enc, fields) => {
    ...
    const createStackConfig = (stackMode, baseConfig) => {
    ...
    const applyStacking = (markFunction, data, baseConfig, stackAxis) => {
        if (!stacked || !colorField) {
            ...
            if (primaryField) {
                ...
                if (primaryFieldType === 'temporal') {
                    ...
                    if (a[primaryField] < b[primaryField])
                        primaryCompare = -1;
                    else if (a[primaryField] > b[primaryField])
                        primaryCompare = 1;
                    else
                        primaryCompare = 0;
                }
    ...
    const createMark = (markType, data, enc, fields) => {
    ...
    let mark = createMark(markType, data, enc, fields);
    // 6) Title / tooltip
    ...
    if (tooltipEnc) {
        if (Array.isArray(tooltipEnc)) {
            ...
            titleFn = (d) => tooltipEnc.map((tt) => `${tt.field}: ${d[tt.field]}`).join(', ');
        }
        else if (tooltipEnc.field) {
            // e.g. tooltip: {"field": "colA"}
            titleFn = (d) => `${tooltipEnc.field}: ${d[tooltipEnc.field]}`;
        }
    ...
    const isTemporalUsedCategorically = (axis) => {
...
export function toObservablePlotSpec({ rows: rowsRaw, columns: columnsRaw, color, opacity, size, shape, theta, radius, text, details = [], interactiveScale, dataSource, layoutMode, width, height, defaultAggregated, geomType, stack, scales, mediaTheme, vegaConfig, displayOffset, }) {
