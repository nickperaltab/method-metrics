...
const TopTree = (props) => {
    ...
    const nodeCells = useMemo(() => {
        ...
        cellRows.push(new Array(totalChildrenSize).fill(0).flatMap((ele, idx) => measInCol.map((m) => (React.createElement("td", { key: `${cellRows.length}-${m.fid}-${m.aggName}-${idx}`, className: "bg-secondary text-secondary-foreground whitespace-nowrap p-2 text-xs m-1 border" }, m.titleOverride || getMeaAggName(m.name, m.aggName, m.windowAgg))))));
        cellRows.shift();
        return cellRows;
