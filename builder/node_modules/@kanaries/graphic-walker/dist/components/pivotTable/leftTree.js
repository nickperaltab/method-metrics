    ...
    const nodeCells = useMemo(() => {
        ...
        if (measInRow.length > 0) {
            ...
            for (let row of cellRows) {
                ans.push([
                    ...row,
                    React.createElement("td", { key: `0-${measInRow[0].fid}-${measInRow[0].aggName}`, className: "bg-secondary text-secondary-foreground whitespace-nowrap p-2 text-xs m-1 border" }, measInRow[0].titleOverride || getMeaAggName(measInRow[0].name, measInRow[0].aggName, measInRow[0].windowAgg)),
                ]);
                for (let j = 1; j < measInRow.length; j++) {
                    ans.push([
                        React.createElement("td", { key: `${j}-${measInRow[j].fid}-${measInRow[j].aggName}`, className: "bg-secondary text-secondary-foreground whitespace-nowrap p-2 text-xs m-1 border" }, measInRow[j].titleOverride || getMeaAggName(measInRow[j].name, measInRow[j].aggName, measInRow[j].windowAgg)),
                    ]);
                }
            }
            return ans;
