...
export function serializeManualKey(value) {
    if (value === null)
        return MANUAL_NULL_KEY;
    if (value === undefined)
        return MANUAL_UNDEFINED_KEY;
...
export function createManualSortLookup(config) {
    if (!config)
        return undefined;
    const lookup = {};
...
const createSortToken = (fieldKey, keyValue, nodeData, isLeaf, sort, manualSortLookup, alphabeticalSort) => {
    ...
    if (isLeaf && sort) {
        ...
        return { priority: 1, value: normalizeSortValue(fallback) };
    }
    if (alphabeticalSort) {
        return { priority: 2, value: normalizeSortValue(keyValue) };
    }
...
function insertNode(tree, layerKeys, nodeData, depth, collapsedKeyList, sort, manualSortLookup, alphabeticalSortConfig) {
...
function binarySearchIndex(arr, keyVal, reverse = false) {
...
function compareSortToken(a, b, reverse = false) {
...
function insertSummaryNode(node) {
...
export function buildNestTree(layerKeys, data, collapsedKeyList, showSummary, sort, dataWithoutSort, manualSortLookup, alphabeticalSortConfig) {
    const tree = {
        key: ROOT_KEY,
        value: 'root',
        fieldKey: 'root',
...
export function buildMetricTableFromNestTree(leftTree, topTree, data) {
    const mat = [];
...
export function getAllChildrenSize(node, depth) {
    if (depth === 0) {
