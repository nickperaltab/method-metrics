...
const PivotTable = function PivotTableComponent(props) {
    ...
    const manualSortConfig = useMemo(() => {
        const entries = {};
        const collect = (field) => {
            if ((field.sortType ?? 'measure') !== 'manual')
                return;
            if (!field.sortList || field.sortList.length === 0)
                return;
    ...
    const alphabeticalSortConfig = useMemo(() => {
        const entries = {};
        const collect = (field) => {
            if ((field.sortType ?? 'measure') !== 'alphabetical')
                return;
    ...
    const generateNewTable = () => {
        ...
        buildPivotTableService(dimsInRow, dimsInColumn, [...measInRow, ...measInColumn], data, aggData.current, Object.keys(tableCollapsedHeaderMap), showTableSummary, sort !== 'none' && sortedEncoding !== 'none'
            ? {
                fid: sortedEncoding === 'column'
            ...
            : undefined, manualSortConfig, alphabeticalSortConfig)
            .then((data) => {
            ...
            React.createElement("table", { className: "border border-collapse" },
                React.createElement("thead", { className: "border" },
                    ...
                    topTreeHeaderRowNum > 0 && (React.createElement("tr", { className: "" },
                        dimsInRow.map((x) => (React.createElement("td", { className: "bg-secondary text-secondary-foreground p-2 m-1 text-xs border whitespace-nowrap", colSpan: 1 }, x.titleOverride || x.name))),
