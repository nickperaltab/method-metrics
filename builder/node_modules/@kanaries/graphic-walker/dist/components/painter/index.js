...
import { ClickInput, ColorEditor, PixelContainer } from './components';
...
function selectInteractive(scene) {
    const selected = [];
    let index = 0;
    function visit(item) {
...
const getFactor = (f) => f.domain.type === 'nominal' ? (GLOBAL_CONFIG.PAINT_SIZE_FACTOR * GLOBAL_CONFIG.PAINT_MAP_SIZE) / f.domain.width : GLOBAL_CONFIG.PAINT_SIZE_FACTOR;
function getMarkForAgg(types) {
...
const getDomainType = (type) => (type === 'quantitative' ? 'quantitative' : 'nominal');
const deduper = function (items, keyF) {
    const map = new Map();
    items.forEach((x) => map.set(keyF(x), x));
    return [...map.values()];
};
const getAggDimensionFields = (fields) => {
    return deduper([fields.x, fields.y, fields.color, fields.size, fields.opacity, fields.shape]
        .filter((x) => !!x)
        .filter((x) => x.analyticType === 'dimension'), (x) => x.fid);
};
const produceChannel = (channel, domain, options) => {
    if (domain.domain.value.every((x) => x instanceof Array)) {
        return {
            ...
            scale: {
                domain: domain.domain.type === 'nominal' && options?.reverseNominalDomain
...
const produceAggChannel = (channel, domain, options) => {
    if (domain?.domain.value.every((x) => x instanceof Array)) {
        return {
            field: `${channel.fid}[0]`,
            title: channel.titleOverride || channel.name,
...
const AggPainterContent = (props) => {
    ...
    const fields = useMemo(() => [props.x, props.y, props.color, props.size, props.opacity, props.shape].filter((x) => x).map((x) => x.field), [props.x, props.y, props.color, props.size, props.opacity, props.shape]);
    const [viewDimensions, viewMeasures] = useMemo(() => {
        const totalFields = deduper(fields.concat(props.facets.flatMap((x) => x.dimensions.map((d) => ({
            fid: d.fid,
            name: d.fid,
    ...
    const paintDimensions = useMemo(() => {
        return deduper([props.x, props.y, props.color, props.size, props.opacity, props.shape].filter((x) => x).filter((x) => x.domain), (x) => x.field.fid).map((x) => x.domain);
    }, []);
    ...
    useEffect(() => {
        if (!loading && containerRef.current) {
            ...
            Object.values(spec.encoding).forEach((c) => {
                if (!c)
                    return;
                if (c.aggregate === null)
                    return;
            ...
            embed(containerRef.current, spec, {
                ...
                const scene = res.view.scenegraph();
                ...
                interactive.forEach((item) => sceneVisit(item, (item) => {
                    if ('datum' in item) {
                ...
                const handleDraw = (e) => {
                    ...
                    e.preventDefault();
                    const paint = (x, y) => {
                        const targetColor = brushIdRef.current === ERASER
                            ? {
                                name: '',
                                color: 'transparent',
                            }
                    ...
                    if (e instanceof MouseEvent && e.buttons & MouseButtons.PRIMARY) {
                        ...
                        paint(e.changedTouches[0].pageX - rect.left - origin[0] - MAGIC_PADDING, e.changedTouches[0].pageY - rect.top - origin[1] - MAGIC_PADDING);
                    }
                };
    ...
    return (React.createElement("div", { className: "flex" },
        React.createElement(PixelContainer, { color: props.dict[brushId]?.color ?? '#333', cursor: cursor, offsetX: pixelOffset[0], offsetY: pixelOffset[1], showPreview: showCursorPreview },
            React.createElement("div", { ref: containerRef, id: "painter-container", className: "!cursor-none min-w-[600px] min-h-[512px]" })),
        React.createElement("div", { className: "flex flex-col space-y-2 pt-10 w-40 flex-shrink-0 px-1" },
            React.createElement(Tabs, { value: brushId === ERASER ? 'eraser' : 'palette', onValueChange: (v) => {
                    if (v === 'palette') {
                        setBrushId(2);
                    }
                    else {
                ...
                React.createElement(TabsList, null,
                    React.createElement(TabsTrigger, { value: "palette" },
                        " ",
                        t('main.tabpanel.settings.paint.palette')),
                    React.createElement(TabsTrigger, { value: "eraser" },
                        " ",
                        t('main.tabpanel.settings.paint.eraser'))),
                brushId !== ERASER && (React.createElement(TabsContent, { value: "palette" },
                    React.createElement("div", { className: "grid grid-cols-5 gap-2 p-2" }, Object.entries(props.dict).map(([id, { color }]) => (React.createElement("div", { key: id, className: `box-border rounded-full border-primary hover: ${id === `${brushId}` ? 'border-2' : 'hover:border-2'} active:ring-ring active:ring-1 w-4 h-4`, style: {
                            background: color,
                        }, onClick: () => setBrushId(Number(id)) })))),
                    React.createElement("div", { className: "flex space-x-2" },
                        React.createElement("label", { className: "block text-xs font-medium" }, t('main.tabpanel.settings.paint.color')),
                        React.createElement(ColorEditor, { color: props.dict[brushId].color, colors: scheme, onChangeColor: (color) => {
                                props.onChangeDict({
                                    ...props.dict,
                                    [brushId]: {
                                        color,
                    ...
                    React.createElement("div", { className: "flex flex-col space-y-2" },
                        React.createElement("label", { className: "block text-xs font-medium" }, t('main.tabpanel.settings.paint.label')),
                        React.createElement(ClickInput, { value: props.dict[brushId].name, onChange: (name) => {
                                props.onChangeDict({
                                    ...props.dict,
                                    [brushId]: {
            ...
            React.createElement("div", { className: "pt-2" },
                React.createElement("output", { className: "text-sm" },
                    t('main.tabpanel.settings.paint.brush_size'),
                    ": ",
                    `${brushSize}`),
                React.createElement(Slider, { className: "mt-2", value: [brushSize], min: GLOBAL_CONFIG.PAINT_MIN_BRUSH_SIZE, max: GLOBAL_CONFIG.PAINT_MAX_BRUSH_SIZE, onValueChange: ([v]) => setBrushSize(v) })),
            React.createElement("div", { className: "flex-1 flex flex-col space-y-2 justify-end" },
                React.createElement(Button, { variant: "destructive", children: t('main.tabpanel.settings.paint.delete_paint'), onClick: props.onDelete }),
                React.createElement(Button, { variant: "outline", children: t('main.tabpanel.settings.paint.reset_paint'), onClick: () => resetRef.current() }),
                React.createElement(Button, { variant: "outline", children: t('main.tabpanel.settings.paint.cancel'), onClick: props.onCancel }),
                React.createElement(Button, { children: t('main.tabpanel.settings.paint.save_paint'), onClick: props.onSave })))));
};
...
const PainterContent = (props) => {
    ...
    ].concat(props.facets.flatMap((x) => x.dimensions.map((d) => ({ fid: d.fid, name: d.fid, semanticType: d.domain.type, analyticType: 'measure' })))), (x) => x.fid), [props.x, props.y]);
    const brushSizeRef = useRef(GLOBAL_CONFIG.PAINT_DEFAULT_BRUSH_SIZE);
    const [brushSize, setBrushSize] = useState(GLOBAL_CONFIG.PAINT_DEFAULT_BRUSH_SIZE);
    brushSizeRef.current = brushSize;
    const brushIdRef = useRef(2);
            ...
            embed(containerRef.current, spec, {
                ...
                const handleDraw = (e) => {
                    ...
                    const paint = (x, y) => {
                        ...
                        if (!targetColor)
                            return;
                        const pts = getCircleIndexes([Math.floor(x / getFactor(props.domainX)), props.domainY.domain.width - 1 - Math.floor(y / getFactor(props.domainY))], brushSizeRef.current, [props.domainY, props.domainX]);
                        let i = 0;
                        pts.forEach((x) => {
                            itemsMap.get(x)?.forEach((item) => {
    ...
    const cursor = useMemo(() => {
        ...
        return {
            ...
            xFactor: props.domainX.domain.type === 'nominal'
            ...
            yFactor: props.domainY.domain.type === 'nominal'
    ...
    return (React.createElement("div", { className: "flex" },
        ...
        React.createElement("div", { className: "flex flex-col space-y-2 pt-10 w-40 flex-shrink-0 px-1" },
            ...
            (props.domainX.domain.type === 'quantitative' || props.domainY.domain.type === 'quantitative') && (React.createElement("div", { className: "pt-2" },
                React.createElement("output", { className: "text-sm" },
                    t('main.tabpanel.settings.paint.brush_size'),
                    ": ",
                    `${brushSize}`),
...
function isZeroscaled([min, max]) {
    return (min < 0 && max > 0) || min === 0 || max === 0;
}
function isDomainZeroscaledAs(domain, exceptedZeroscale) {
    if (!domain)
        return false;
...
const Painter = ({ themeConfig, themeKey }) => {
    ...
    useEffect(() => {
        if (showPainterPanel) {
            ...
            (async () => {
                ...
                const getNewMap = async (paintInfo) => {
                ...
                if (paintInfo) {
                    if (paintInfo.type === 'exist') {
                        ...
                        if (paintInfo.new.type === 'new') {
                            ...
                            if (lastFacet.dimensions[0]?.fid !== paintInfo.new.y.fid ||
                            ...
                            if (!getAggDimensionFields(paintInfo.new).every((f, i) => {
    ...
    const saveMap = async () => {
        const dimensions = domainX && domainY
            ? [domainY, domainX]
            : aggInfo
    ...
    return (React.createElement(Dialog, { open: showPainterPanel, onOpenChange: () => {
            vizStore.setShowPainter(false);
        } },
        React.createElement(DialogContent, null,
            loading && React.createElement(LoadingLayer, null),
            !loading && !aggInfo && (React.createElement(PainterContent, { mark: geoms[0], vegaConfig: vegaConfig, onSave: saveMap, onDelete: () => {
                    vizStore.updatePaint(null, '');
                    vizStore.setShowPainter(false);
                }, onCancel: () => {
                    vizStore.setShowPainter(false);
                }, x: fieldX, y: fieldY, facets: facets, allFields: allFields, domainX: domainX, domainY: domainY, dict: dict, onChangeDict: setDict, paintMapRef: paintMapRef, onReset: onReset, displayOffset: timezoneDisplayOffset })),
            !loading && aggInfo && (React.createElement(AggPainterContent, { mark: geoms[0], vegaConfig: vegaConfig, onSave: saveMap, onDelete: () => {
                    vizStore.updatePaint(null, '');
                    vizStore.setShowPainter(false);
                }, onCancel: () => {
                    vizStore.setShowPainter(false);
                }, x: aggInfo.x, y: aggInfo.y, color: aggInfo.color, size: aggInfo.size, opacity: aggInfo.opacity, shape: aggInfo.shape, facets: facets, allFields: allFields, dict: dict, onChangeDict: setDict, paintMapRef: paintMapRef, onReset: onReset, displayOffset: timezoneDisplayOffset })))));
};
