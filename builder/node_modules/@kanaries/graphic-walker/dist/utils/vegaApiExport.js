...
export const useVegaExportApi = (name, viewsRef, ref, renderTaskRefs, containerRef) => {
    const renderHandle = {
        getSVGData() {
    ...
    useEffect(() => {
        ...
        if (ctx) {
            ...
            ctx.exportChart = (async (mode = 'svg') => {
                ...
                if (ctx.renderStatus !== 'idle') {
                    ...
                    const waitForChartReady = new Promise((resolve, reject) => {
            ...
            ctx.exportChartList = async function* exportChartList(mode = 'svg') {
    ...
    useEffect(() => {
        ...
        return () => {
            if (appRef.current) {
                ...
                appRef.current.exportChart = async (mode = 'svg') => ({
                    mode,
                    title: '',
                    nCols: 0,
                    nRows: 0,
                ...
                appRef.current.exportChartList = async function* exportChartList(mode = 'svg') {
                    yield {
                        mode,
                        total: 1,
                        completed: 0,
...
//# sourceMappingURL=vegaApiExport.js.map