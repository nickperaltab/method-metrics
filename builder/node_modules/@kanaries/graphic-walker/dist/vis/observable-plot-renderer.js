import React, { useEffect, useState, useMemo, useRef, useContext, forwardRef } from 'react';
...
function toPlotSpec(props) {
    ...
    switch (geomType) {
        case 'bar':
            mark = Plot.barY(dataSource, {
                x: (d) => d[ /* your x field */'category'],
                y: (d) => d[ /* your measure field */'value'],
                fill: color ? (d) => d[color.fid] : undefined,
            });
            break;
        case 'point':
        default:
            mark = Plot.dot(dataSource, {
                x: (d) => d[ /* your x field */'category'],
                y: (d) => d[ /* your measure field */'value'],
                fill: color ? (d) => d[color.fid] : undefined,
            });
 ...
 */
const ObservablePlotRenderer = forwardRef(function ObservablePlotRenderer(props, ref) {
    const { name, rows = [], columns = [], dataSource = [], defaultAggregate = true, stack = 'stack', geomType, color, opacity, size, theta, radius, shape, text, onGeomClick, showActions, interactiveScale, layoutMode, width, height, details = [], locale = 'en-US', useSvg, scales: channelScaleRaw, scale, displayOffset, vegaConfig, onReportSpec, } = props;
    ...
    useEffect(() => {
        ...
        specs.forEach((plotSpec, index) => {
            ...
            plotElem.addEventListener('pointerdown', (e) => {
                click$.next(e);
                onGeomClick?.(null, e); // pass real data as needed
            });
    ...
    React.useImperativeHandle(ref, () => ({
        ...
        downloadSVG: async (filename) => {
            ...
            const data = await ref.current?.getSVGData();
            // you’d implement your own file-saver logic
            return data;
        },
        downloadPNG: async (filename) => {
            const data = await ref.current?.getCanvasData();
            // you’d implement your own file-saver logic
            return data;
        },
    }), []);
    ...
    return (React.createElement("div", { className: layoutMode === 'auto' ? 'w-fit h-fit relative' : 'w-full h-full relative', style: { overflow: layoutMode === 'auto' ? 'visible' : 'hidden' } },
        ...
        React.createElement(CanvasContainer, { style: {
                ...(layoutMode === 'auto' ? {} : { width: '100%', height: '100%' }),
            }, rowSize: Math.max(rowRepeatFields.length, 1), colSize: Math.max(colRepeatFields.length, 1), ref: containerRef }, plotPlaceholders.map((plotRef, i) => (React.createElement("div", { key: i, ref: plotRef, className: layoutMode === 'auto' ? '' : 'overflow-hidden' }))))));
});
export default ObservablePlotRenderer;
//# sourceMappingURL=observable-plot-renderer.js.map