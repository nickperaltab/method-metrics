...
expressionFunction('formatBin', (datum, formatString) => {
...
const leastOne = (x) => Math.max(x, 1);
const ReactVega = forwardRef(function ReactVega(props, ref) {
    const { name, dataSource = [], rows = [], columns = [], defaultAggregate = true, stack = 'stack', geomType, color, opacity, size, theta, radius, shape, text, onGeomClick, showActions, interactiveScale, layoutMode, width, height, details = [], 
    // themeKey = 'vega',
    vegaConfig, 
    // format
    locale = 'en-US', useSvg, scales: channelScaleRaw, scale, displayOffset, } = props;
    ...
    const specs = useMemo(() => toVegaSpec({
        columns: guardedCols,
        dataSource,
    ...
    // Render
    useEffect(() => {
        props.onReportSpec?.(JSON.stringify(specs.map((x) => ({
            ...x,
            data: undefined,
        })), undefined, 4));
        vegaRefs.current = [];
        ...
        if (rowRepeatFields.length <= 1 && colRepeatFields.length <= 1) {
            if (viewPlaceholders.length > 0 && viewPlaceholders[0].current) {
                const task = embed(viewPlaceholders[0].current, specs[0], {
                    ...
                    startTask(() => {
                        const success = layoutMode !== 'auto' || useSvg || (rect && canvasSize.test({ width: rect.renderWidth || 1, height: rect.renderHeight || 1 }));
                        if (!success) {
                            if (canvas) {
            ...
            const combinedParamStore$ = new Subject();
            const throttledParamStore$ = combinedParamStore$.pipe(op.throttleTime((dataSource.length / 64) * rowRepeatFields.length * colRepeatFields.length, undefined, { leading: false, trailing: true }));
            const subscriptions = [];
            const subscribe = (cb) => {
            ...
            for (let i = 0; i < leastOne(rowRepeatFields.length); i++) {
                for (let j = 0; j < leastOne(colRepeatFields.length); j++, index++) {
                    ...
                    const node = i * leastOne(colRepeatFields.length) + j < viewPlaceholders.length
                                ...
                                if (rect) {
                                    ...
                                    if (res.view.width() === 0) {
                                        if (res.view.signal('child_width') !== undefined) {
                                            res.view.signal('child_width', specs[0].width - modifier.width / Math.round((areaWidth || width) / colRepeatFields.length / specs[0].width));
                                            res.view.signal('child_height', specs[0].height -
    ...
    return (React.createElement("div", { className: layoutMode === 'auto' ? 'w-fit h-fit relative' : 'w-full h-full relative', style: { overflow: layoutMode === 'auto' ? 'visible' : 'hidden' } },
        React.createElement("div", { ref: areaRef, className: "inset-0 absolute" }),
        React.createElement(CanvaContainer, { style: {
                ...(layoutMode === 'auto' ? {} : { width: '100%', height: '100%' }),
            }, rowSize: Math.max(rowRepeatFields.length, 1), colSize: Math.max(colRepeatFields.length, 1), ref: containerRef }, viewPlaceholders.map((view, i) => (React.createElement("div", { key: i, ref: view, className: layoutMode === 'auto' ? '' : 'overflow-hidden' }))))));
});
