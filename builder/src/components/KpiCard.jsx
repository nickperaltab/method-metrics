...
export default function KpiCard({ metricName, value, delta, deltaPercent, isRate, isPercent, displayFormat, hasError }) {
  if (hasError) {
    return (
  ...
  const formattedValue = displayFormat === 'decimal_rate' || isRate
    ? `${(value * 100).toFixed(1)}%`
    : (displayFormat === 'percent' || isPercent)
