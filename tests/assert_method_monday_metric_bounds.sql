...
WITH metric_values AS (
  SELECT 'trials_mtd' AS metric, period, value FROM {{ ref('v_metric__trials_mtd') }}
  UNION ALL
  SELECT 'syncs_mtd', period, value FROM {{ ref('v_metric__syncs_mtd') }}
  UNION ALL
  SELECT 'conversions_mtd', period, value FROM {{ ref('v_metric__conversions_mtd') }}
  UNION ALL
  SELECT 'churn_mtd', period, value FROM {{ ref('v_metric__churn_mtd') }}
  UNION ALL
  SELECT 'sync_rate_mtd', period, value FROM {{ ref('v_metric__sync_rate_mtd') }}
  UNION ALL
  SELECT 'trials_trajectory', period, value FROM {{ ref('v_metric__trials_trajectory') }}
  UNION ALL
  SELECT 'syncs_trajectory', period, value FROM {{ ref('v_metric__syncs_trajectory') }}
  UNION ALL
  SELECT 'conversions_trajectory', period, value FROM {{ ref('v_metric__conversions_trajectory') }}
  UNION ALL
  SELECT 'churn_trajectory', period, value FROM {{ ref('v_metric__churn_trajectory') }}
  UNION ALL
  SELECT 'conversions_forecast_mtd', period, value FROM {{ ref('v_metric__conversions_forecast_mtd') }}
  UNION ALL
  SELECT 'churn_forecast_mtd', period, value FROM {{ ref('v_metric__churn_forecast_mtd') }}
  UNION ALL
  SELECT 'sync_conversion_rate_trajectory', period, value FROM {{ ref('v_metric__sync_conversion_rate_trajectory') }}
),
...
full_month_forecast AS (
  SELECT
    ...
    (SELECT SUM(Forecasted_Conversion) FROM {{ source('revenue', 'method_forecast') }}
     WHERE Date IS NOT NULL
       AND DATE_TRUNC(Date, MONTH) = DATE_TRUNC(CURRENT_DATE(), MONTH)) AS full_month_value
  UNION ALL
  SELECT
    'churn_forecast_mtd',
    (SELECT SUM(Forecasted_Churn) FROM {{ source('revenue', 'method_forecast') }}
     WHERE Date IS NOT NULL
