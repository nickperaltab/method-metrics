...
WITH rates AS (
  SELECT 'churn_rate_mtd' AS metric, period, value FROM {{ ref('v_metric__churn_rate_mtd') }}
  UNION ALL
  SELECT 'churn_rate_trajectory', period, value FROM {{ ref('v_metric__churn_rate_trajectory') }}
)
