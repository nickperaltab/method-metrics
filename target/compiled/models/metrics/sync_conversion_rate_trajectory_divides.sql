...
WITH
            	`project-for-method-dw_revenue_metrics_v_metric__conversions_trajectory` as (SELECT *  FROM UNNEST([STRUCT(CAST('2026-07-01' AS DATE) AS period, CAST(71.86 AS FLOAT64) AS value)])),
  	`project-for-method-dw_revenue_metrics_v_metric__syncs_trajectory` as (SELECT *  FROM UNNEST([STRUCT(CAST('2026-07-01' AS DATE) AS period, CAST(250.0 AS FLOAT64) AS value)])),
  	`project-for-method-dw_revenue_metrics_v_metric__sync_conversion_rate_trajectory_expect` as (SELECT *  FROM UNNEST([STRUCT(CAST('2026-07-01' AS DATE) AS period, CAST(0.28744 AS FLOAT64) AS value)])),
