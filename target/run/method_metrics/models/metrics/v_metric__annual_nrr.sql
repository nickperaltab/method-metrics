  ...
  create or replace view `project-for-method-dw`.`revenue_metrics`.`v_metric__annual_nrr`
  OPTIONS(
...
FROM `project-for-method-dw`.`revenue_metrics`.`v_metric__annual_start_mrr` s
JOIN `project-for-method-dw`.`revenue_metrics`.`v_metric__annual_cancellations_mrr` c USING (period)
JOIN `project-for-method-dw`.`revenue_metrics`.`v_metric__annual_downgrades_mrr` d USING (period)
JOIN `project-for-method-dw`.`revenue_metrics`.`v_metric__annual_expansions_mrr` e USING (period)
ORDER BY s.period;
