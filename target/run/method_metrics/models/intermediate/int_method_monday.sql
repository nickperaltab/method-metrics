  ...
  create or replace view `project-for-method-dw`.`revenue`.`int_method_monday`
  OPTIONS(
      description="""One row for the current month carrying every quantity the Method Monday scorecard needs: MTD actuals through yesterday, full-month forecasts, complete-days trajectories, and forecasts prorated to the elapsed window. The elapsed-days arithmetic lives here and nowhere else so the sixteen tiles built on it cannot drift apart. On the 1st of the month elapsed_days is 0 and every trajectory is NULL by design.\n"""
    )
  as 
