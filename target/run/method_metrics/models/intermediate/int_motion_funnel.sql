    ...
    create or replace table `project-for-method-dw`.`revenue`.`int_motion_funnel`
...
censor AS (
  SELECT
    DATE_SUB(DATE_TRUNC(CURRENT_DATE(), MONTH), INTERVAL 1 MONTH) AS censor_month
)
SELECT
  t.EntityRecordID,
  t.signup_month,
...
LEFT JOIN `project-for-method-dw`.`revenue`.`int_presale_touches` pt ON pt.EntityRecordID = t.EntityRecordID
LEFT JOIN `project-for-method-dw`.`revenue`.`int_customer_proserv` ps ON ps.EntityRecordID = t.EntityRecordID
...
LEFT JOIN `project-for-method-dw`.`v7_classification`.`v_entity_primary_label` ind
