    ...
    create or replace table `project-for-method-dw`.`revenue`.`int_customer_retention_triangle`
...
dims AS (  -- cohort-start attributes, one row per entity at its first paying month
  ...
  FROM `project-for-method-dw`.`revenue`.`int_customer_mrr` d
...
base AS (
  ...
  LEFT JOIN `project-for-method-dw`.`v7_classification`.`v_entity_primary_label` lbl ON lbl.customer_record_id = fp.EntityRecordID
),
joined AS (
  SELECT
