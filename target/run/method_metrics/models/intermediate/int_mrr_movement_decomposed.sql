    ...
    create or replace table `project-for-method-dw`.`revenue`.`int_mrr_movement_decomposed`
    ...
    OPTIONS(
      description="""Per (month, entity) month-over-month MRR change split into app_mrr / seat_mrr / price_mrr via a price\u2013volume\u2013mix decomposition, plus a movement_kind label (downgrade / expansion / cancellation / new / flat). Buckets sum to the net change. Validated 2026-06-03: reconciles to int_customer_mrr movement totals at $0.00 across 96 (month, kind) pairs (scripts/parity_mrr_decomposition_vs_customer_mrr.py).\n"""
    )
    as (
