    ...
    OPTIONS(
      description="""One row per (Partner, CompanyAccount) \u2014 the accounts a referring partner brought in. Partner = raw revenue.Account.Partner string (no normalization in v1). IsActive is a lifecycle flag (FirstSaaSInvoiceTxnDate set AND no CancellationDate), matching the partner CRM \"Active?\" view (validated to an exact match against a reference partner's CRM export). MRR and Licenses are point-in-time billing for the latest complete month; they are 0 for accounts that did not bill that month \u2014 including hard-hold accounts that are still lifecycle-active. Excludes the internal \"Method Integration\" partner and m11/m18 test accounts. Account is deduped to one row per CompanyAccount. See spec docs/superpowers/specs/2026-06-25-partner-referral-views-design.md.\n"""
    )
    as (
